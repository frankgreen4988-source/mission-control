      ...
      <div className="mb-8">
        ...
        <div className="bg-slate-800 p-6 rounded-lg">
          ...
          <div className="grid grid-cols-3 gap-3 mb-3">
            ...
            <input
              type="datetime-local"
              onChange={(e) => setNewEvent({ ...newEvent, startTime: new Date(e.target.value).getTime() })}
              className="bg-slate-700 text-white px-4 py-2 rounded"
            />
            <select
              value={newEvent.assignedTo}
              onChange={(e) => setNewEvent({ ...newEvent, assignedTo: e.target.value as any })}
              className="bg-slate-700 text-white px-4 py-2 rounded"
            >
            ...
            <div key={date} className="bg-slate-800 rounded-lg p-6">
              ...
              <div className="space-y-2">
                {dayEvents.map((event: any) => (
                  <div
                    key={event._id}
                    className={`${typeColors[event.type]} p-4 rounded flex justify-between items-start`}
                  >
                    ...
                    <div className="flex gap-2">
                      <button
                        onClick={() => updateEvent({ id: event._id, isCompleted: !event.isCompleted })}
