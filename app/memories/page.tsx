          ...
          {newMemory.tags.length > 0 && (
            <div className="flex gap-2 mb-3 flex-wrap">
              {newMemory.tags.map((tag) => (
                <span key={tag} className="bg-slate-600 px-3 py-1 rounded text-sm">
                  {tag}{" "}
                  <button
        ...
        {filteredMemories?.map((memory: any) => (
          <div key={memory._id} className="bg-slate-800 rounded-lg p-6 border border-slate-700">
            <div className="flex justify-between items-start mb-3">
              ...
              <div className="flex gap-2">
                <button
                  onClick={() => updateMemory({ id: memory._id, isPinned: !memory.isPinned })}
                  className={memory.isPinned ? "text-yellow-400" : "text-slate-400"}
                >
                  <Pin size={18} />
                </button>
                <button
            ...
            <div className={`inline-block px-3 py-1 rounded text-sm font-semibold mb-3 ${categoryColors[memory.category]}`}>
              {memory.category}
            </div>
            ...
            {memory.tags.length > 0 && (
              <div className="flex gap-2 flex-wrap mb-3">
                {memory.tags.map((tag: string) => (
                  <span key={tag} className="bg-slate-700 px-2 py-1 text-xs text-slate-300 rounded">
                    #{tag}
                  </span>
                ))}
              </div>
