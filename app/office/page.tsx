...
export default function OfficePage() {
  ...
  const officeStatus = useQuery(api.team.getOfficeStatus);
  const updateOfficeStatus = useMutation(api.team.updateOfficeStatus);

  const [selectedMember, setSelectedMember] = useState<string | null>(null);

  const getMemberStatus = (memberId: string) => {
    return officeStatus?.find((s) => s.memberId === memberId);
  };

  const getRandomDesk = () => {
    return deskPositions[Math.floor(Math.random() * deskPositions.length)];
  };

  const handleAssignDesk = async (memberId: string) => {
    const desk = getRandomDesk();
          ...
          <div className="relative w-full h-full bg-gradient-to-br from-slate-700 to-slate-900 rounded">
            ...
            {deskPositions.map((desk) => {
              const member = members?.find(
                (m) => getMemberStatus(m._id)?.location === desk.label
              );
              return (
                <div
                  key={desk.id}
                ...
                >
                  <div className="w-20 h-20 bg-slate-700 rounded-lg border-2 border-slate-600 flex items-center justify-center cursor-pointer hover:border-blue-500 transition"
                    onClick={() => member && setSelectedMember(member._id)}
                  >
                    {member ? (
                      <div className="text-center">
                        <p className="text-3xl mb-1">{member.avatar}</p>
                        <p className="text-xs text-slate-400">{member.name}</p>
                      </div>
                    ) : (
                      <div className="text-center">
                        <p className="text-2xl">💼</p>
                        <p className="text-xs text-slate-500">{desk.label}</p>
                      </div>
                    )}
                  </div>
                </div>
          ...
          <div className="space-y-3">
            {members?.map((member: any) => {
              ...
              return (
                <div
                  key={member._id}
                  className={`bg-slate-700 p-4 rounded-lg border-2 transition cursor-pointer ${
                    isSelected ? "border-blue-500" : "border-slate-600 hover:border-slate-500"
                  }`}
                  onClick={() => setSelectedMember(isSelected ? null : member._id)}
                >
                  ...
                  {isSelected && (
                    <div className="mt-3 pt-3 border-t border-slate-600 space-y-2">
                      <div>
                        ...
                        <p className="text-sm font-semibold">{status?.location || "Not assigned"}</p>
                      </div>
                      {status?.currentTask && (
                        <div>
                      ...
                      <div className="grid grid-cols-2 gap-2 mt-3">
                        <button
                          onClick={() => handleAssignDesk(member._id)}
          ...
          {officeStatus
            ...
            .map((activity: any) => (
              <div key={activity._id} className="text-sm text-slate-300">
                <span className="font-semibold text-white">{activity.member?.name}</span> is{" "}
                <span className="text-blue-400">{activity.status}</span> at{" "}
                <span className="text-green-400">{activity.location}</span>{" "}
                <span className="text-slate-500">
                  ({new Date(activity.lastUpdate).toLocaleTimeString()})
                </span>
              </div>
            ))}
        </div>
