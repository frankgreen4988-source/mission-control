...
export default function TeamPage() {
  ...
  const groupedByRole = Object.fromEntries(
    ["lead", "developer", "writer", "designer", "agent"].map((role) => [
      role,
    ...
    <div className="p-8">
      <div className="mb-8">
        ...
        <div className="bg-slate-800 p-6 rounded-lg">
          ...
          <textarea
            ...
            className="w-full bg-slate-700 text-white px-4 py-2 rounded mb-3 h-16"
          />
          ...
          {newMember.skills.length > 0 && (
            <div className="flex gap-2 mb-3 flex-wrap">
              {newMember.skills.map((skill) => (
                <span key={skill} className="bg-slate-600 px-3 py-1 rounded text-sm">
                  {skill}
                  <button
                    onClick={() =>
                      setNewMember({
      ...
      <div className="space-y-8">
        {Object.entries(groupedByRole).map(([role, teamMembers]) => (
          <div key={role}>
            <h2 className={`text-2xl font-bold mb-4 ${roleColors[role]} px-4 py-2 rounded inline-block`}>
              ...
              {teamMembers.map((member: any) => (
                <div key={member._id} className="bg-slate-800 rounded-lg p-6 border border-slate-700">
                  <div className="flex justify-between items-start mb-3">
                    ...
                    <button
                      onClick={() => deleteMember({ id: member._id })}
                    ...
                    <div className="mb-3">
                      ...
                      <div className="flex gap-2 flex-wrap">
                        {member.skills.map((skill: string) => (
                          <span key={skill} className="bg-slate-700 px-2 py-1 text-xs rounded">
                            {skill}
                          </span>
                        ))}
                      </div>
                  ...
                  <div className="text-xs text-slate-500 pt-3 border-t border-slate-700 mt-3">
                    {member.type === "subagent" ? "🤖 Subagent" : "👤 Human"}
                  </div>
                </div>
              ))}
            </div>
